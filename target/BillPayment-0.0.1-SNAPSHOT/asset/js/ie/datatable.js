$.fn.sgdsTable = function(config){
	...
	var renderHeaderTable = function(){
		...
		if(arrSort.length){
			...
			arrSort.removeClass();
			$.each(arrSort, function (index, item) {
	...
	var _table = function(_params){
		...
		var rawData = function(){
			...
			if(dataList.length < 1){
				$.each(elementTBody, function ($ind, $row) {
					$row.remove();
				});
				return;
			}
			...
			if(elementTBody.length < 1 ){
				...
				dataList.forEach(function(item){
					...
					Object.keys(columnDefs).forEach(function (col) {
...
//				leftFoot = 'Page 0 of 0 ';
				...
				if(totalPages == 0){
					...
					for (var i = 0; i < tmpCount; i++) {
						...
						if($pageDom.length == 0){
							...
							var tmpRow = '<span class="' + classClocked + '" style="display: inline-block;" id="tblPage">' + page + '</span>'
	...
	var _getData = function(_val){
		var _data = dataList.filter(function (item) {
