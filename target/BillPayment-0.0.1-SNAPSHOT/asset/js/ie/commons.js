...
function generateActionBar(url, container, appStatus) {
	var jsonData = {'appStatus':appStatus};
...
function renderActionBar(container, data) {
	...
	var fixedTop = $('<div class="modal-header button-fixed-top">');
	for(i=0; i< data.length; i++) {
		var btnData = JSON.parse(data[i].button);
		var btn = $('<button type="button" class="sgds-button is-link is-rounded ml-1">');
