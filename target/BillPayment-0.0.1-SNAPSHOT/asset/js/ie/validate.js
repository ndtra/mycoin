...
function setMaxLengthWithCounterForRequire(element, max){
	var maxLength = parseInt(max);
	var maxChar = parseInt(max);
	var ele = $(element);
	var eleOrg = element.replace('#','').replace('.','');
...
$.fn.requirePositiveNumber = function requirePositiveNumber() {
	...
	function fnIsValid() {
		var value = ele.val();
		value = value.replace(/[,.]/g, "");
		if (isNaN(value) || hasWhiteSpace(value) || fnSign(value) < 0) {
			return false;
		}
		return true;
	}
...
$.fn.formatUen = function formatUen(){
	...
	function fnIsValid() {
        ...
        if(arr1.length == arr.length){
        	var valid = arr1.map(function(item) {
...
$.fn.dasForm = function() {
	...
	function clearError() {
		...
		for (var inp in lstInput) {
			$(lstInput[inp]).dataError().clear();
		}
		
		var lstSelectMess = $('form#' + eleOrg + ' .selectTagErr');
		for (var inp in lstSelectMess) {
			$(lstSelectMess[inp]).remove();
		}
	}
...
$.fn.dasDataSegment = function() {
	...
	function clearError() {
		...
		for (var ele in lstInput) {
			$(lstInput[ele]).dataError().clear();
		}
		
		var lstSelect = $('#' + eleOrg + ' .selectTagErr');
		for (var ele in lstSelect) {
			$(lstSelect[ele]).remove();
		}
	}
...
function checkValidForm(formId){
	...
	for (var ele in lstInput) {
		var err = $(lstInput[ele]).dataError().hasError();
		if (err == true) {
			lstInput[ele].focus();
			return false;
		}
